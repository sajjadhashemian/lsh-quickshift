...
def __plot(res):
    fig, ax = plt.subplots(
        len(res), 7, figsize=(12, len(res) + 1), sharex=True, sharey=True
    )
    ...
    for row, img in enumerate(res):
        ...
        for col, alg in enumerate(res[row]):
            ...
            ax[row, col + 1].set_title(
                f"{alg.name}, #: {len(np.unique(alg.lable))}, (s): {alg.t:.2f}",
                fontsize=7,
            )
...
def segment(img, _c):
    ...
    segments_ms = MeanShift(
        bandwidth=estimate_bandwidth(image, n_samples=100),
        bin_seeding=True,
        max_iter=400,
    ).fit(image)
